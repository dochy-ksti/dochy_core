...
pub fn json_item_to_rust(name : &str, value_type : VarType, v : &JVal, names : &Names) -> CoreResult<RustValue> {
...
pub fn json_item_to_rust_ref(name : &str, value_type : VarType, v : &JVal, names : &Names) -> CoreResult<RustValue> {
    let names = &names.append(name);
    match v {
        JVal::Bool(_, span) => {
