...
// use crate::HashS;
//
// pub fn validate_compatible(source_def : &ListDefObj, compatible : &HashS<String>, root : &RootObject, can_use_old : bool, names : &Names) -> Result<()>{
...
//                 }
//                 if let Some(value) = root.default().prepare(name){
...
//     }
//
//     let value = if let Some(item) = current_def.default().prepare(name) { item } else {
